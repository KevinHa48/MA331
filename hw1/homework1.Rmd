...
To calculate the following `qnorm()` will be used with $\mu = 0$ and $\sigma = 1$

\

$P (X \leq z_{0.05})$, given $\alpha = 0.05$, the z-score $1-\alpha$ needs to be found.

```{r}
qnorm(1-0.05, 0, 1)
```

$P (X \leq z_{0.05}) = P(Z \leq 1.644854) = 0.95$

\

$P (X \leq z_{1-0.05/2})$, given $\alpha = 1-0.05/2$, the z-score $1-\alpha$ needs to be found.

```{r}
qnorm(1-0.975, 0, 1)
```
$P (X \leq z_{1-0.05/2}) = P(Z \leq -1.959964) = 0.025$

\

$P (z_{0.05/2} \leq X \leq z_{1-0.05/2})$

$P (z_{0.05/2})$ given $\alpha = 0.05/2$, the z-score $1-\alpha$ needs to be found.
```{r}
qnorm(1- 0.025, 0, 1)
```
$P (X \leq z_{1-0.05/2}) = P(Z \leq 1.959964) = 0.975$
...
5. $\frac{\sum_{i=1}^{n} x_i}{n}$ is the population mean and can be reduced to just $\overline{x}$ \
